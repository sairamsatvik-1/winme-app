...
function normalizeJudgeTables(text) {
  ...
  const rows = tsvLines.map((row) => row.split("\t").map((c) => c.trim()));
...
export default function MessageBubble({ role, text, isStreaming }) {
  ...
  // -----------------------------
  // Normal user/ai bubbles
  // -----------------------------
  return (
    <div
      className={`flex ${isUser ? "justify-end pr-20" : "justify-start"} my-1`}
    >
      <div
      ...
      >
        <ReactMarkdown
          ...
          components={{
            p: ({ children }) => (
              <p
                className={`mb-3 last:mb-0 leading-relaxed text-[13px] ${
            ...
            ul: ({ children }) => (
              <ul className="list-disc pl-6 mb-3 space-y-1 text-gray-100">
                {children}
              </ul>
            ),
            ol: ({ children }) => (
              <ol className="list-decimal pl-6 mb-3 space-y-1 text-gray-100">
                {children}
              </ol>
            ),
            li: ({ children }) => (
